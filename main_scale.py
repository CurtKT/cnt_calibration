...
class Pyqt5_Serial(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.time_2 = QtCore.QTimer()
        self.time_3 = QtCore.QTimer()
        self.time_4 = QtCore.QTimer()
        self.time_5 = QtCore.QTimer()
        self.ser = serial.Serial()
        self.time_3.timeout.connect(self.port_check)
        self.time_3.start(300)
    ...
    def port_open(self):
        ...
        try:
            ...
            return None
        self.textBrowser.append("建立通信中...")
        if self.ser.isOpen():
            self.data_receive()
        self.i = 40
        self.time_confirm_link = QtCore.QTimer()
        self.time_confirm_link.timeout.connect(self.confirm_link)
        self.time_confirm_link.start(100)

    def confirm_link(self):
            self.i = self.i - 1
            self.input_s = "12 34 0f 00 03 00 08 00 00 ab cd"  # 发送查询是否开机
            self.send()
            time.sleep(0.1)
            if self.i == 1:
                self.textBrowser.append("通信失败，请检查是否开机,并等待5秒！")
                self.time_confirm_link.stop()
                self.ser.close()
                return
            if self.ser.in_waiting:
                ...
                self.get_num()  # 接收计数值
                self.over_flag = None
                self.over_flag_2 = None
                self.time_5.timeout.connect(self.overtime)
                self.time_5.start(1000)
                self.time_confirm_link.stop()
                return
    ...
    def star(self):
        ...
        self.isstar = True
        self.time_4.timeout.connect(self.twinkle)
        self.time_4.start(150)
    ...
    def get_num(self):
        """循环接收信息"""
        self.time_2.timeout.connect(self.circul_send)
        self.time_2.start(100)
        ...
        if self.ser.isOpen():
            try:
                if self.ser.in_waiting:
                    ...
                    time_s = float(time_ns/10**9)  # 纳秒转化为秒
                    time_s = float('%.1f' % time_s)
                    ...
                    # if self.spinBox.value() == time_s and self.stop_flag == 1:
                        self.isstar = False
                        self.pushButton_2.setEnabled(False)
                        self.pushButton_3.setEnabled(False)
                        self.stop_flag = 0
                        ...
                        self.textBrowser.append("测量时间%ds" % self.spinBox.value())
                        self.textBrowser.append("测量阈值%dmV" % self.spinBox_2.value())
                        if self.checkBox.isChecked():
                            self.textBrowser.append("设定高压%dV" % self.spinBox_3.value())
                        else:
    ...
    def overtime(self):
        if self.isstar:
            try:
                self.over_flag, self.over_flag_2 = self.msg, self.over_flag
                if self.over_flag == self.over_flag_2:
                    self.isstar = False
    ...
    def data_receive(self):
        """接收数据多线程"""
        self.time.timeout.connect(self.receive)
        self.time.start(50)
    ...
    def set_time(self):
        ...
        if len(str_time) == 1:
            ...
            send_time = self.text_time[1:x_index] + "00 00 0" + str_time + self.text_time[x_r_index+1:-1]
        elif len(str_time) == 2:
            # send_time = "12 34 0f 00 06 14 00 00 " + str_time + " ab cd"
            send_time = self.text_time[1:x_index] + "00 00 " + str_time + self.text_time[x_r_index+1:-1]
        elif len(str_time) == 3:
            # send_time = "12 34 0f 00 06 14 00 0" + str_time[0:1] + " " + str_time[1:] + " ab cd"
            send_time = self.text_time[1:x_index] + "00 0" + str_time[0:1] + " " + str_time[1:] \
                        + self.text_time[x_r_index+1:-1]
        elif len(str_time) == 4:
            # send_time = "12 34 0f 00 06 14 00 " + str_time[0:2] + " " + str_time[2:] + " ab cd"
            send_time = self.text_time[1:x_index] + "00 " + str_time[0:2] + " " +\
                        str_time[2:] + self.text_time[x_r_index+1:-1]
        elif len(str_time) == 5:
            # send_time = "12 34 0f 00 06 14 00 " + str_time[0:2] + " " + str_time[2:] + " ab cd"
            send_time = self.text_time[1:x_index] + "0" + str_time[0:1] + " " +str_time[1:3] + " " +\
                        str_time[3:] + self.text_time[x_r_index+1:-1]
        elif len(str_time) == 6:
            # send_time = "12 34 0f 00 06 14 00 " + str_time[0:2] + " " + str_time[2:] + " ab cd"
            send_time = self.text_time[1:x_index] + str_time[0:2] + " " + str_time[2:4] + " " + str_time[4:] \
                        + self.text_time[x_r_index+1:-1]
    ...
    def set_voltage(self):
        ...
        if self.checkBox.isChecked():
            ...
            self.send()
            self.textBrowser.append("设置高压：%dV" % (self.spinBox_3.value()))
            self.spinBox_3.setEnabled(False)
